...
const WhatsAppAdmin = () => {
  ...
  const fetchData = async () => {
    ...
    if (data) setWsData(data);
    setLoading(false);
  };

  const handleSave = async () => {
    setSaving(true);
    const { error } = await supabase.from("whatsapp_settings").upsert(wsData);
    if (error) {
      ...
      <Card>
        ...
        <CardContent className="grid gap-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Phone Number (e.g. 628...)</label>
            <Input value={wsData.phone_number} onChange={(e) => setWsData({...wsData, phone_number: e.target.value})} />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">Default Message</label>
            <Input value={wsData.default_message} onChange={(e) => setWsData({...wsData, default_message: e.target.value})} />
          </div>
          <div className="space-y-2">
            <label className="text-sm font-medium">Float Button Text</label>
            <Input value={wsData.button_text} onChange={(e) => setWsData({...wsData, button_text: e.target.value})} />
          </div>
        </CardContent>
      </Card>
    </div>
